from pysb import Model, Monomer, Parameter, Initial, Observable, Rule, Expression
...
# Define Monomers
Monomer("NADH", ["state", "b"], {"state": ["free", "bound"]})
Monomer("Ox", ["b"])
Monomer("NAD_plus", ["b"])
Monomer("Inhibitor", ["type"], {"type": ["oxamate", "rot", "oligo", "fccp"]})

# Define Parameters
Parameter("NADH_free_init", 50)
Parameter("NADH_bound_init", 50)
Parameter("Ox_init", 50)
Parameter("NAD_plus_init", 50)
Parameter("Inhibitor_init", 50)
Parameter("alpha", 0.1)
Parameter("beta_eq", 0.1)
...
Rule("NADH_binding",
     ...
     Parameter("k_bind", 1e-2))

Rule("NADH_unbinding",
     NADH(state="bound", b=1) % Ox(b=1) >>
     NADH(state="free", b=None) + Ox(b=None),
     Parameter("k_unbind", 1e-2))
